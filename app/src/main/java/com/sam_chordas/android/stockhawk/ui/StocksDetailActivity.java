...
/**
 * Activity showing the detailed graph of prices of a stock overtime.
 */
...
public class StocksDetailActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        change=(TextView)findViewById(R.id.price_change);

        companySymbol = getIntent().getStringExtra("symbol");
        symbol.setText(companySymbol);
        if(getIntent().hasExtra("price")){
            stock_price=getIntent().getStringExtra("price");
            price.setText(stock_price);
        }

        if(getIntent().hasExtra("change")){
            stock_change=getIntent().getStringExtra("change");
            change.setText(stock_change);
            if(stock_change.startsWith("+"))
                change.setTextColor(Color.parseColor("#00E676"));
            else
                change.setTextColor(Color.parseColor("#ff6347"));
        }
    ...
    private void onDownloadCompleted() {
        StocksDetailActivity.this.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                series.setColor(Color.parseColor("#BBDEFB"));
